// Importing the Project model to interact with the database
...
export const createProject = async (req, res) => {
  const { title, description, technologies, github, image, category } = req.body; // Extracting data from the request body
  ...
  try {
    ...
    res.status(201).json({ message: 'Project created successfully', project }); // Respond with success
  } catch (error) {
...
export const getProjects = async (req, res) => {
  try {
    ...
    res.status(200).json(projects); // Respond with the retrieved data
  } catch (error) {
...
export const updateProject = async (req, res) => {
  ...
  try {
    ...
    if (!project) {
      return res.status(404).json({ message: 'Project not found' }); // Respond if not found
    }
    ...
    res.status(200).json({ message: 'Project updated successfully' }); // Respond with success
  } catch (error) {
...
export const deleteProject = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Project deleted successfully' }); // Respond with success
  } catch (error) {
